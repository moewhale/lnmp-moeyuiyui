...
Install_ImageMagic()
{
    ...
    if [ -s /usr/local/imagemagick/bin/convert ]; then
        ...
        if echo "${Cur_PHP_Version}" | grep -Eqi '^5.2.';then
            Download_Files https://download.imagemagick.org/archive/releases/ImageMagick-6.9.9-51.tar.xz ImageMagick-6.9.9-51.tar.xz
            Tar_Cd ImageMagick-6.9.9-27.tar.gz ImageMagick-6.9.9-27
        else
            Download_Files https://download.imagemagick.org/archive/releases/${ImageMagick_Ver}.tar.xz ${ImageMagick_Ver}.tar.xz
